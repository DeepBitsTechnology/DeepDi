...
class DisassemblyText(ctypes.Structure):
    ...
    def __str__(self):
        return f'<"{self.text}", {self.address:16X}>'
...
class FileData(ctypes.Structure):
    ...
    def disassemble(self, start: int, end: int, return_string: bool) -> DisassemblyResult:
        v = DisassemblyResult()
...
class Library:
    def __init__(self):
        ...
        self.FileData_Disassemble.argtypes = [ctypes.POINTER(FileData), ctypes.POINTER(DisassemblyResult), ctypes.c_int64, ctypes.c_int64, ctypes.c_bool]
    ...
    def open(self, path: bytes) -> FileData:
        v = FileData()
